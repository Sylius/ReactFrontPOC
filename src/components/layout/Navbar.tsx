...
const Navbar: React.FC = () => {
    ...
    useEffect(() => {
        const fetchTaxons = async () => {
            try {
                const res = await fetch(`${process.env.REACT_APP_API_URL}/api/v2/shop/taxons`);
                    ...
                    data.map(async (taxon) => {
                        const childObjects: TaxonChild[] = await Promise.all(
                            taxon.children.map(async (childUrl) => {
                                const res = await fetch(`${process.env.REACT_APP_API_URL}${childUrl}`);
...
eeeeeeeeeeee                                    <div className="dropdown-menu position-relative position-lg-absolute border-0 border-lg dropdown-custom">
