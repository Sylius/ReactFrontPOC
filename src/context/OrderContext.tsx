...
const pickupCart = async () => {
    try {
        const response = await fetch(`${process.env.REACT_APP_API_URL}/api/v2/shop/orders`, {
            method: "POST",
            headers: {
...
const updateOrderItemAPI = async ({ id, quantity }: { id: number; quantity: number }) => {
    const response = await fetch(`${process.env.REACT_APP_API_URL}/api/v2/shop/orders/${localStorage.getItem("orderToken")}/items/${id}}`, {
        method: "PATCH",
...
const removeOrderItemAPI = async (id: number) => {
    const response = await fetch(`${process.env.REACT_APP_API_URL}/api/v2/shop/orders/${localStorage.getItem("orderToken")}/items/${id}`, { method: "DELETE" });
...
export const OrderProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    return (
        <OrderContext.Provider value={{ order: order || [], fetchOrder: refetch, updateOrderItem, removeOrderItem, isFetching, orderToken, setOrderToken }}>
            {children}
