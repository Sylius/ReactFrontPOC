...
const AddressPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchAddressesAndCountries = async () => {
      ...
      try {
        ...
        const items = addressData['hydra:member'] as AddressInterface[];
        setAddresses(items);
        ...
        const defaultAddress = items.find((addr: AddressInterface) => String(addr.id) === defaultAddressId);
        if (defaultAddress) {
  ...
  const handleAddressSelect =
      (setter: React.Dispatch<React.SetStateAction<AddressInterface>>) =>
          (e: React.ChangeEvent<HTMLSelectElement>) => {
            ...
            const selected = addresses.find((addr: AddressInterface) => String(addr.id) === selectedId);
            if (selected) {
              setter({ ...selected });
            }
          };
